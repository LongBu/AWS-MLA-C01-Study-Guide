...
  * Tagging SM resources/user profiles within a SM Studio domain allows cost tracking per user or team. AWS Budgets can then monitor these tags and send alerts when usage exceeds a defined threshold.
  * SM Studio Notebooks provide persistent storage, allowing users to manage multiple notebooks, support multi-user domain support, store datasets, and access them later allowing better management of ML projects
  ...
  * SM Notebook instances are the better alternative for single users providing managed infrastructure, EBS backed storage persistence and less overhead configuration
  * SM Studio Notebooks require more setup: domain, user profile configuration, shared EFS storage and other configuration
...
## SM Automatic Model Tuning
  * To initiate an automatic hyperparameter tuning job in SM, one must select to supply the CreateHyperParameterTuningJob API:
    ...
    * Objective metric
  * Can use managed spot training via EnableManagedSpotTraining
  * Can add a stopping condition via StoppingCondition in the training job configuration
